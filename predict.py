...
parser = ArgumentParser(description='Evaluate the embeddings.')
...
parser.add_argument(
    '--batch_size', default=15, type=common.positive_int,
    help='Batch size used during evaluation, adapt based on your memory usage.')#was something then to 80 then to 32
...
def main():
    ...
    # Prediction Function
    predictFunction(scores, gallery_pids, pids)

def predictFunction(scores, gallery_pids, pids):
    print(scores, '\n', gallery_pids, '\n', pids)
    for i in range(len(scores)):

        print(pids[i], 'is', 
            gallery_pids[np.ndarray.tolist(scores[i]).index(max(scores[i]))], 
            'with a score of',
            max(scores[i]) )
